...
const compute = ComputeBudgetProgram.setComputeUnitPrice({
...
export class Amm {
  ...
  async createFundTx(pubkeys, jitoTipLamports, blockhash) {
    ...
    const vTx = await createVtxWithOnlyMainSigner(
      [compute, compute2, createFundTxIx],
      this.payer,
      blockhash
    );
    return vTx;
  ...
  async swapMakers(direction, mint, signer, blockhash, excludes) {
    const wSolAta = await getOwnerAta(
    ...
    if (
      !this.jupiterCache.buy ||
      !this.jupiterCache.sell ||
      now - this.jupiterCache.lastUpdateTime > 10000
    ) {
        ...
        try {
          ...
          if (!buyResponse) {
            ...
            if (!buyResult.error) {
              ...
              this.log(
                `No buy route at ${baseAmount} lamports: ${buyResult.error}`
              );
            }
          }
          ...
          if (buyResponse && !sellResponse && outAmount) {
            ...
            if (!sellResult.error) {
              ...
              this.log(
                `No sell route at ${sellAmount} tokens: ${sellResult.error}`
              );
...
• Sell amount: ${outAmount * 3} tokens`);
      ...
      // Cache the routes
      this.jupiterCache.buy = await fetch(
      ...
      this.jupiterCache.sell = await fetch(
        "https://quote-api.jup.ag/v6/swap-instructions",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    let rawJupResponse =
      direction === "buy" ? this.jupiterCache.buy : this.jupiterCache.sell;
    const tables = await getLookupTables(
      rawJupResponse.addressLookupTableAddresses,
      this.connection
    );
    ...
    const vTx = await createVtx(
      [compute, compute2, swapIx],
      [signer, this.payer],
      tables,
      blockhash
    );
...
${
  includeDexes.length > 0
    ? `• Enabled DEXes: ${includeDexes.join(", ")}`
    : "• Using all available DEXes"
}
===========================
`);
    ...
    const testVtx = await createVtxWithOnlyMainSigner(
      [testSend],
      this.payer,
      blockhash
    );
    ...
    while (stats.makersCompleted < totalMakersRequired) {
      try {
        ...
        const signers = Array(4)
          .fill()
          .map(() => Keypair.generate());
        const fundTx = await this.createFundTx(
          signers,
          jitoTipLamports,
          blockhash
        );
        const firstBuyTx = await this.swapMakers(
          "buy",
          mint,
          signers[0],
          blockhash,
        ...
        const secondBuyTx = await this.swapMakers(
          "buy",
          mint,
          signers[1],
          blockhash,
        ...
        const thirdBuyTx = await this.swapMakers(
          "buy",
          mint,
          signers[2],
          blockhash,
        ...
        const sellTx = await this.swapMakers(
          "buy",
          mint,
          signers[3],
          blockhash,
...
`);
  ...
  async swapVolume(direction, mint, amount, blockhash, signer, excludes) {
    ...
    let tables;
    try {
      tables = await getLookupTables(
        rawJupResponse.addressLookupTableAddresses,
        this.connection
      );
    } catch (E) {
      this.log(
        "Error in the amount or possibly unsupported token. Try increasing the amount."
      );
    }
    if (tables) {
      const rawSwapIx = rawJupResponse.swapInstruction;
...
• Expected Net Volume: ${(expectedBuyVolume - expectedSellVolume).toFixed(
      4
    )} SOL
...
`);
    ...
    while (true) {
      try {
        ...
        const fundTx = await this.createFundSingleTx(
          signer,
          jitoTipLamports,
          blockhash
        );
        ...
        const bundle = await sendBundle(
          [bs58.encode(fundTx.serialize()), bs58.encode(swapTx.serialize())],
          getRandomJitoUrl()
        );
...
• Bundle: ${bundle.result}`);
  ...
  async swap(mint, direction, amount, options = {}) {
    ...
    try {
      ...
      const swapIx = await this.swapVolume(
        direction,
        mint,
        amount * 10 ** 9,
        blockhash,
      ...
      const send = await sendBundle(
        [bs58.encode(fundTx.serialize()), bs58.encode(swapIx.serialize())],
        getRandomJitoUrl()
      );
      return;
    } catch (E) {
